...
export default function MukDialog({title, content, children, visible, onReject, onAccept, labelReject, labelAccept, isLoading}: Props) {
  ...
  return (
    <Dialog visible={visible} onDismiss={onDismiss} style={[{backgroundColor: theme.colors.background}, styles.shadow]}>
      ...
      <Dialog.Actions style={{gap: responsiveWidth(16)}}>
        <MukButton onPress={onDismiss} label={labelReject ?? 'Geri Dön'} buttonStyle={{backgroundColor: theme.colors.error}} scale={.8}
                   textStyle={{color: theme.colors.secondary, fontSize: responsiveSize(14), fontWeight: '600'}}/>
        <MukButton loading={isLoading} onPress={onAccept} label={labelAccept ?? 'Onayla'} buttonStyle={{backgroundColor: theme.colors.primary}} scale={.8}
                   textStyle={{color: theme.colors.secondary, fontSize: responsiveSize(14), fontWeight: '600'}}/>
      </Dialog.Actions>
    </Dialog>
  );
