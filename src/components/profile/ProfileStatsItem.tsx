...
export default function ProfileStatsItem({stat, index, activeIndex, setActiveIndex}: Props) {
  ...
  return (
    <TouchableOpacity onPress={() => setActiveIndex(index)} style={{
      ...
      <Text style={{fontSize: responsiveSize(14), fontWeight: 'bold', color: colors.onSurfaceVariant}}>{stat.label}</Text>
    </TouchableOpacity>
  );
}
