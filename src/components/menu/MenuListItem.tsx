...
export default function MenuListItem({item}: Props) {
  ...
  return (
    <MukListItem onPress={() => item.disabled ? soonTitle() : navigation.navigate(item.route)} style={{alignItems: 'center', paddingVertical: responsiveWidth(2)}}>
      <MukIcon scale={0.8} icon={item.icon} color={item.disabled ? title.color : colors.secondary} />
      <Text numberOfLines={1} style={{fontSize: responsiveSize(20), fontWeight: '600', marginLeft: responsiveWidth(-8), color: item.disabled ? title.color : colors.secondary}}>
        {title.label}
      </Text>
    </MukListItem>
  );
